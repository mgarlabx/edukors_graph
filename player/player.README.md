...
2. **Configuration.** Copy `private/config.sample.php` to `private/config.php` and fill it in: the database, the OpenRouter key and model, an admin password hash, and `base_url`.
   ...
   If your courses judge — if any of them has a `choice`, `score` or `noul` node — fill in `judge.model` and `judge.min_confidence` as well. A course names no model: this server names the one that answers every judgement, as an exact version and never an alias, and how sure a judgement has to be to count. With `judge.model` empty, judgements do not happen and every student leaves those nodes by their unconditional edge. See [Judgements](#judgements).
...
| -------- | ---------------------------------------- | --------------------------------------------------------------------- |
| `choice` | `choice`, `confidence`, `probabilities`  | `<id>.<key>`, `-confidence`                                           |
| `score`  | `score`, `confidence`, `probabilities`   | `<id>.<key>`, `-confidence`, `-points`, and the spread as `~spread`   |
...
`-confidence` is not worked out from the spread — the model answers that question itself, and the floor in `judge.min_confidence` means that number directly: when any question of a node comes back under it, the whole node counts as not judged. A `noul` has no confidence and takes no floor: the probability it returns is already the measure of how sure the model is.

The spread is not a key of the schema, and no course can read it. A score's is kept under `<id>.<key>~spread` — a name no course can write, since a question's key is lowercase letters, digits and hyphens — for the node that writes from the judgement, which shows every level with its share of the weight. The levels themselves are read from the author's `criteria` in the course, never from the `legend` the answer echoes back, so no round trip can reword a course. A choice's spread is checked and then dropped, because nothing reads it; for a student, the whole answer stays in `node_state.verdict` all the same.
...
**What this server is honest about.** A course names no model. This server names one, in `judge.model`, and answers every judgement of every course with it — so changing it moves the thresholds of every course at once, and is worth a run of `tools/judge-probe.php` on a real node of each course first. `ai_call.model` logs what actually answered, and with no model named the judgements simply do not happen rather than being answered by the generation model. The `-confidence` a decisions model returns is its own answer rather than a number read back out of a distribution, which makes a floor worth more than it used to be — but it is still worth watching the `-confidence` values on the student pages of a real cohort before settling on `judge.min_confidence`.
